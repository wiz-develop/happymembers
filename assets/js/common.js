jQuery(function ($) {
    ...
    function createCalendar(year, month) {
        ...
        minmonth = new Date(date.getFullYear(), date.getMonth()-2, 1); //先々月判定
          ...
          if(eventdate == pickeddate){
            ...
            if (terms) {
              ...
              terms.forEach(function(term) {
                terms_list += '<div class="article-detail__category '+term.slug+'"><span>'+term.name+'</span></div>';
              });
            }
            ...
            item += '</div></div><div class="article-title"><p>'+value.post_title+'</p></div></div>';
                ...
                item += '<div class="modal_block">';
                  ...
                  item += '<div class="event-content">';
                    item += '<div class="event-content__detail">';
                      item += '<div class="event-content__detail__header"><div class="article-detail"><div class="category-list">';
                        if (terms) {
                          item += terms_list;
                        }
                        item += '</div></div><div class="article-title"><p>'+value.post_title+'</p></div>';
                      item += '</div>';
                      item += '<table>';
                        item += '<tbody>';
                          if (value.post_date) {
                            var date = new Date( value.post_date );
                            item += '<tr>';
                              item += '<td>開催日</td>';
                              item += '<td>'+date.getFullYear() + '/' + ('00' + (date.getMonth()+1)).slice(-2) + '/' + ('00' + date.getDate()).slice(-2) +'('+weeks[date.getDay()]+')'+'</td>';
                            item += '</tr>';
                          }
                          if(value.event_time) {
                          item += '<tr>';
                          ...
                          item += '<tr>';
                            item += '<td>開催状況</td>';
                            item += '<td><span class="status-'+Object.keys(value.event_status)+'">'+Object.values(value.event_status)+'</span></td>';
                          item += '</tr>';
                          }
                          if (value.meeting_division) {
                          item += '<tr>';
                            item += '<td>会合区分</td>';
                            item += '<td>'+value.meeting_division+'</td>';
                          item += '</tr>';
                          }
                  ...
                  item += '<div class="release-day">掲載日：<time datetime="'+value.publish_date+'">'+value.publish_date+'</time></div>';
    ...
    $(document).ready(function(){
      $("#registration-mail").emailautocomplete({
        domains: domainData
      });
      $("#registration-mail-confirm").emailautocomplete({
        domains: domainData
      });
    });
});
